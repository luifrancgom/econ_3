...
- This presentation is based on [@blanchardMacroeconomics2017a, Chapter 19]
...
- Classification of __de facto__ exchange arrangements [@imfAnnualReportExchange2020, p 43-45]:
...
- __De facto__ exchange arrangements around the world [@imfAnnualReportExchange2020]

```{r, out.width = "75%"}

read_csv(file = "010_AREAERDataQueryReport_ Exchange_Arrangement (1949 -)_2020_08_27.csv") %>% 
    select(Year, Country, Code, Index, Category, Status) %>% 
    filter(Year %in% 2010:2018,
           Code %in% c(971, 972, 973, 974, 159, 975, 976, 977, 978, 979),
           Status %in% "yes") %>% 
    count(Year, Category) %>% 
    ...
    ggplot(aes(x    = Year, 
               y    = n, 
               fill = fct_reorder(.f = Category, .x = n) %>% 
                        fct_rev())) +
    ...
    labs(y        = "Number of countries",
         title    = "De facto exchange rate arrangements around the world",
         caption  = str_glue("Source: IMF AREAER Database
                             Last date update: 2020-08"), 
         fill     = "") +
...
# Data 1: clasiffication exchange rate regimes
classif <- tibble(regime = c("Crawling peg", 
                             "Pegged exchange rate within horizontal bands",
                             "Free floating") %>% as_factor(),
                  start  = c(ymd("1967-03-01"), ymd("1991-10-01"), ymd("1999-11-01")),
                  end    = c(ymd("1991-10-01"), ymd("1999-11-01"), ymd("2020-08-01")))

# Data 2: monthly average exchange rate COP/USD
...
col_cop_usd_monthly_mean %>%     
    ...
    geom_rect(data  = classif,
              ...
              show.legend = TRUE) +
    scale_fill_tq() +
    scale_x_date(breaks = c(ymd("1967-03-01"), 
                            ymd("1991-10-01"),
                            ymd("1999-11-01"),
                            ymd("2020-08-01")), 
                 date_labels = "%Y-%m") +

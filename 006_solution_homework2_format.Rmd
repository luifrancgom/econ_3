...
monetary_basse_col <- read_csv("006_depositos_en_el_sistema_financiero_PSE.csv", 
    ...
    # Plot
static_plot1 <- monetary_basse_col %>% 
    ggplot(aes(x = date, y = value, group = variable)) +
    geom_line(aes(color = variable)) + 
    geom_point(aes(text = label_text, fill = variable), 
               color = "black",
               shape = 21) +
    geom_line() +
    labs(x        = "Year",
...
static_plot2 <- i_daily %>% 
    ...
    labs(x        = '', 
         y        = 'Percent',
         title    = 'Monetary policy intervention rate 
         of the Bank of the Republic (Colombia)') +
    scale_y_continuous(labels = scales::percent_format(accuracy = 0.1), 
                       breaks = seq(from = 0, to = 0.25, by = 0.025)) + 
    expand_limits(y = 0) +
    theme(panel.border      = element_rect(fill = NA, color = "black"),
          plot.background   = element_rect(fill = "#f3fcfc"),
          panel.background  = element_rect(fill = "#f3f7fc"),
...
static_plot3 <- col_islm_real_nocommercial %>% 
    ...
    labs(x        = "Year",
         y        = "Billions (Long scale)",
         title    = "Colombia real domestic aggregate demand components 
         and real GDP") +
    theme(panel.border      = element_rect(fill = NA, color = "black"),
          plot.background   = element_rect(fill = "#f3fcfc"),
          panel.background  = element_rect(fill = "#f3f7fc"),
...
Initially, if the central bank increases the money supply, $(\frac{M}{P})^s$, then also the central bank must decrease $i_t$ so people will accept to have money in their pockets. Therefore the **LM** will be affected taking into account that it is defined as $i_t = \overline{i}_t$. This effect can be seen later when we solve the next point where $\frac{di_t}{d(\frac{M}{P})^s} = -\frac{1}{4000} < 0$. 

Also the **IS** curve will be affected because the increases in the money supply, $(\frac{M}{P})^s$, implies a change in the interest rate, $i_t$, that affects investment, $I_t$. Because investment, $I_t$ will change then the **IS** will be affected and the direction of the change will depend on how investment is affected because  $\frac{dIS_t}{dI_t} > 0$.

**Observation**: This exercise is interesting because it shows how difficult is to think when you have to understand the dynamics of an economic model where different variables are interacting and affecting each other. If you solve the next point you will see that if $(\frac{M}{P})^s$ changes from $1200$ to $1500$ then, taking into account **the overall equilibrium** of the IS-LM model, $Y_t$ will change from $820$ t0 $795$ and $i_t$ will change from $0.01 (1\%)$ to $0.0225 (2.25\%)$. 

Initially we said that if $(\frac{M}{P})^s$ increases then $i_t$ must decrease. However as you will see in the next point $i_t$ will depend also on $Y_t$. If $(\frac{M}{P})^s$ increases also $Y_t$ will increase but the effect of this 2 variables goes in opposite directions. In the next point you will see that the increase of $Y_t$ will be greater than the increase in $(\frac{M}{P})^s$, $2∗795 > 1500$. Therefore, taking into account **the overall equilibrium** of the IS-LM model $Y_t$ will decrease and $i_t$ will increase when $(\frac{M}{P})^s$ increases.

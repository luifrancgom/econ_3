---
title: "The Board of Directors' Report to the Congress of Colombia (March 2020)"
date: 2020-09-10
output:
  beamer_presentation:
    colortheme: dolphin
...
- This presentation is based on [@bancodelarepublicaInformeJuntaDirectiva2020, Chapter 2]
...
\tiny
- Real GDP growth [@bancodelarepublicaInformeJuntaDirectiva2020, p 33, table 2.1]
...
\tiny
- Real GDP growth by sectors [@bancodelarepublicaInformeJuntaDirectiva2020, p 34, table 2.2]
...
[^5]: An explanation about, $\widehat{T}_t$, can be found in [@unitednationsNationalAccountsPractical2004, Chapter 1, p 5, Item 1.5]

# Observations about data of the Labor Market used in the report

\tiny
- Unemployment rate [@bancodelarepublicaInformeJuntaDirectiva2020, p 37, plot 2.2]

```{r, out.width = '60%'}

include_graphics(path = '011_Grafico_2-2a.png')
```

# Observations about data of the Labor Market used in the report

\tiny
- Employment rate [@bancodelarepublicaInformeJuntaDirectiva2020, p 37, plot 2.2]

```{r, out.width = '60%'}

include_graphics(path = '011_Grafico_2-2b.png')
```
...
[^6]: For more information about the methods used please check [@beedagumSeasonalAdjustmentMethods2016]

# Observations about data of the Labor Market shown in the report

- The data used to generate **"Gráfico 2. Tasa de desempleo y de ocupación por dominios (trimestre móvil desestacionalizado)"** can be examine in:
...
vars <- read_excel(path      = "011_anexo_empleo_jul_20-1.xlsx", 
           sheet     = 2, 
           range     = "A14:IB17", 
           col_names = FALSE) %>% 
    slice(3:4) %>% 
    set_names(c("variable", seq.Date(from = ymd("2001-01-01"), 
                                     to   = ymd("2020-07-01"), by = "1 month") %>% as.character())) %>% 
...
vars_season_adj <- read_excel(path      = "011_anexo_desestacionalizado_empleo_jul_20.xlsx", 
                              sheet     = 2, 
                              range     = "A14:IB16", 
                              col_names = FALSE) %>% 
    slice(2:3) %>% 
...
# Data clean
vars_vs_vars_season_adj_roll <- vars %>% 
    ...
    mutate(er_season_adj_roll3 = rollmean(er_season_adj, 
           ...
           ur_season_adj_roll3 = rollmean(ur_season_adj, 
                                          ...
                                          fill  = NA)) %>%
    drop_na() %>% 
    select(date, er, ur, er_season_adj_roll3, ur_season_adj_roll3)

# Plot ur vs ur_season_adj_roll3 
vars_vs_vars_season_adj_roll %>% 
    ...
    labs(x        = "",
         ...
         title    = str_glue("Total monthly unemployment rate (UR) vs 
                             total  monthly unemployment rate (UR) seasonally adjusted quarterly moving average"),
         subtitle = str_glue("Period: from {vars_vs_vars_season_adj_roll$date[1] %>% format(format = '%Y-%m') } to {vars_vs_vars_season_adj_roll$date[length(vars_vs_vars_season_adj_roll$date)] %>% format(format = '%Y-%m')}
                             Data points: {nrow(vars_vs_vars_season_adj_roll)}"),
    ...
    scale_x_date(breaks      = seq.Date(from = vars_vs_vars_season_adj_roll$date[1],
                                        to = vars_vs_vars_season_adj_roll$date[length(vars_vs_vars_season_adj_roll$date)], 
                                        by = "1 year"),
                 date_labels = "%Y-%m", limits = c(vars_vs_vars_season_adj_roll$date[1], 
                                                   vars_vs_vars_season_adj_roll$date[length(vars_vs_vars_season_adj_roll$date)]))  +
    scale_color_tq() +
    scale_fill_tq() +
...
# Plot er vs er_season_adj_roll3 
vars_vs_vars_season_adj_roll %>% 
    select(date, er, er_season_adj_roll3) %>% 
    pivot_longer(cols     = er:er_season_adj_roll3, 
                 names_to = "variable", values_to = "value") %>%
    mutate(variable = case_when(
        variable == "er" ~ "Total employment rate (ER)",
        variable == "er_season_adj_roll3" ~ "Total employment rate (ER)\nseasonally adjusted quarterly moving average",
        TRUE ~ variable)) %>% 
    ggplot(aes(x = date, 
               y = value)) + 
    geom_line(aes(color = variable, 
    ...
    labs(x        = "",
         y        = "Percent",
         color    = "",
         title    = str_glue("Total monthly employment rate (ER) vs 
                             total  monthly employment rate (ER) seasonally adjusted quarterly moving average"),
         subtitle = str_glue("Period: from {vars_vs_vars_season_adj_roll$date[1] %>% format(format = '%Y-%m') } to {vars_vs_vars_season_adj_roll$date[length(vars_vs_vars_season_adj_roll$date)] %>% format(format = '%Y-%m')}
                             Data points: {nrow(vars_vs_vars_season_adj_roll)}"),
         caption  = str_glue("Source: Departamento Admnistrativo Nacional de Estadística - DANE
                             Last update date: 2020-08-31")) +
...
\tiny
- Inflation [@bancodelarepublicaInformeJuntaDirectiva2020, p 41, table 2.3]

...
**http://www.banrep.gov.co/** > Estadísticas > Precios e inflación > 1. Inflación al consumidor > Índice de precios al consumidor (IPC) > Descargar y consultar:Series IPC total nacional e inflación (desde 07/1954)

```{r}

infla_monthly <- read_excel(path  = "007_ipc_serie_variaciones-2.xlsx", 
           sheet = 1, 
           range = "A13:B806") %>% 
    ...
    drop_na() %>% 
    filter_by_time(.date_var = date, .start_date = "1980-01-01")
```
...
**Observation 2**: If the calculations are correct, you will get similar results as in the column "Inflación anual %".
...
infla_monthly %>% 
    mutate(infla = scales::percent(infla, accuracy = 0.01),
           date  = date %>% as.character() %>% str_sub(start = 1, end = 7)) %>% 
    set_names(c("Date", "IPC (Base 2018-12 = 100)", "Inflation"))
```
...
i_mode_monthly %>% 
    mutate(i = i %>% scales::percent(accuracy = 0.01)) %>% 
    set_names(nm = c("Date", "i"))
```
...
r_approx %>% 
    ...
    mutate_if(.predicate = is.numeric, .funs = scales::percent, accuracy = 0.01) %>% 
...
r_approx_exact <- r_approx %>%
                    mutate(r_exact = ((1 + i) / (1 + lead(infla, n = 12))) - 1) %>% 
                    drop_na()
...
- The financial crisis of 2008 in the USA generated a shift of the **IS** curve to the left where equilibrium production went from $\widehat{Y}^*$ to $\widehat{Y}^{**}$ as shown in the graph below. To mitigate the effects of the crisis, the Obama administration used fiscal policy by cutting taxes, $\widehat{T}$, and increasing the public spending, $\widehat{G}$, causing the **IS** curve to shift to the right. 
